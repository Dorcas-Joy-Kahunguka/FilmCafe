...
$(function () {
	...
	var uploadct = $("#upload-content");
	var loginWrap = $(".login-wrapper");
	var overlay = $(".overlay");
	loginWrap.each(function () {
		$(this).wrap('<div class="overlay"></div>')
	});
	//pop up for upload form
	uploadLink.on('click', function (event) {
		event.preventDefault();
		uploadct.parents(overlay).addClass("openform");
		$(document).on('click', function (e) {
			var target = $(e.target);
			if ($(target).hasClass("overlay")) {
				$(target).find(uploadct).each(function () {
	...
	signupLink.on('click', function (event) {
		...
		// console.log(signupct.parents(overlay).addClass("openform"));
		$(document).on('click', function (e) {
			var target = $(e.target);
			if ($(target).hasClass("overlay")) {
	...
	// If there exists any form sign up error, then display the sign up modal
	if (signup_error.length != 0 || login_error.length != 0 || upload_error.length != 0 || login_request == 'login') {
		...
		if (login_error.length != 0 || login_request == 'login') {
